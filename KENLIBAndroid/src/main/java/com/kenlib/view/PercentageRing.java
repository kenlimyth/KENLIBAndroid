...
public class PercentageRing extends View {
	...
	public PercentageRing(Context context, AttributeSet attrs) {
		...
		// 中间圆的背景颜色 默认为浅紫色
		mCircleBackground = typedArray.getColor(
				R.styleable.PercentageRing_circleBackground, 0xffafb4db);
		// 外圆环的颜色 默认为深紫色
		mRingColor = typedArray.getColor(R.styleable.PercentageRing_ringColor,
				0xff6950a1);
		// 中间圆的半径 默认为60
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawArc(mArcRectF, mStartSweepValue, mCurrentAngle, false,
				mArcPaint);
		// 画文字
		canvas.drawText(String.valueOf(mCurrentPercent) + "%", mCircleX,
				mCircleY + mTextSize / 4, mTextPaint);
		// 判断当前百分比是否小于设置目标的百分比
		if (mCurrentPercent < mTargetPercent) {
			...
			// Utility.showLogTest("onDraw1111111111");
		}

		// Utility.showLogTest("onDraw");
	...
	public String getTargetPercent() {
		return String.valueOf(this.mTargetPercent);
	}
